...
document.addEventListener('DOMContentLoaded', async () => {
	...
	if (gitlabToken) {
		ELEMENTS.inputToken.value = gitlabToken;
	}
	const { gitlabDomain } = await chrome.storage.local.get("gitlabDomain");
	if (gitlabDomain) {
		ELEMENTS.inputDomain.value = gitlabDomain;
	}
});
...
async function registerCustomDomain(userDomain) {
	return new Promise((resolve, reject) => {
		...
		chrome.permissions.remove({ origins: [urlPattern] }, async (removed) => {
			if (removed) {
			...
			reject("Permissions could not be removed (or weren't granted in the first place).");
		});
	});
}